...
def build_run_estimate(cfg: AuditConfig, models: list[str]) -> RunEstimate:
    probes_by_model = {model: len(applicable_probes(model, cfg.all_targeted, cfg.probes_config)) for model in models}
    probe_requests = sum(probes_by_model.values())
    max_output_tokens = 0
    for model, probe_count in probes_by_model.items():
        token_budget = reasoning_token_budget(cfg.api) if is_reasoning_model(model) else cfg.api.max_tokens
        max_output_tokens += probe_count * token_budget
    return RunEstimate(models=list(models), probes_by_model=probes_by_model, probe_requests=probe_requests, max_output_tokens=max_output_tokens)
...
def format_run_estimate(estimate: RunEstimate, output_dir: str | None = None, save_report: bool | None = None, include_ping: bool = False) -> list[str]:
    ...
    lines = [
        ...
        "Estimated dollar cost: unknown (pricing is not configured).",
    ]
    if estimate.models:
...
class TuiReporter(Reporter):
    ...
    def localize_info(self, text: str) -> str:
        ...
        replacements = [
            ...
            ("Estimated dollar cost: unknown (pricing is not configured).", "预估费用：未知（未配置价格表）。"),
